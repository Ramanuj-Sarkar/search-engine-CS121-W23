    ...
    def create_index(self):
        ...
        for root, dirs, files in os.walk(target_pages):
            ...
            for file in files:
                ...
                # find files that exactly match these tokens
                checksum = sum(sum(ord(letter) for letter in token) for token in tokens)
                if checksum in exact_token_match:
                    continue
                exact_token_match.add(checksum)
                self.num_pages += 1
